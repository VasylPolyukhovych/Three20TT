    ...
    if (totalTextWidth > maxTextWidth) {
      ...
      for (int i = 0; i < _tabViews.count; ++i) {
        ...
        CGFloat textWidth = tab.width - kPadding*2;
		  tab.frame = CGRectMake(x, 0, ceil(textWidth * shrinkFactor) + kPadding*2 , self.transform.b == -1 ? self.width : self.height);
        x += tab.width;
      }
    } else {
      ...
      if (maxTabWidth > averageTabWidth && self.width - totalTabWidth < kTabMargin) {
        for (int i = 0; i < _tabViews.count; ++i) {
          TTTab* tab = [_tabViews objectAtIndex:i];
          tab.frame = CGRectMake(x, 0, tab.width, self.transform.b == -1 ? self.width : self.height);
          x += tab.width;
        }
      } else {
        for (int i = 0; i < _tabViews.count; ++i) {
          TTTab* tab = [_tabViews objectAtIndex:i];
          tab.frame = CGRectMake(x, 0, averageTabWidth, self.transform.b == -1 ? self.width : self.height);
          x += tab.width;
        }
      }
    }
