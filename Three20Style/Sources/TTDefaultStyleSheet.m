...
- (TTStyle*)tab:(UIControlState)state {
  if (state == UIControlStateSelected) {
    ...
    return
      ...
      [TTTextStyle styleWithFont:[UIFont boldSystemFontOfSize:14]  color:RGBCOLOR(255,243,194)
                   minimumFontSize:8 shadowColor:nil
                   shadowOffset:CGSizeMake(0, 0) next:nil]]]]]]];
  } else {
    return
      [TTInsetStyle styleWithInset:UIEdgeInsetsMake(5, 0, 1, 1) next:
      [TTBoxStyle styleWithPadding:UIEdgeInsetsMake(6, 12, 2, 12) next:
	  [TTTextStyle styleWithFont:[UIFont boldSystemFontOfSize:14]  color:RGBCOLOR(255,243,194)
				  minimumFontSize:8 shadowColor:nil
					 shadowOffset:CGSizeMake(0, 0) next:nil]]];
...
- (TTStyle*)tabRound:(UIControlState)state {
  if (state == UIControlStateSelected) {
    return
      ...
      [TTTextStyle styleWithFont:[UIFont boldSystemFontOfSize:13]  color:RGBCOLOR(255,243,194)
